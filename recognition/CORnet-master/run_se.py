...
parser.add_argument('--model', choices=['Z', 'R', 'RT', 'S', 'Z_SE'], default='Z',
...
class ImageNetTrain(object):
    ...
    def __call__(self, frac_epoch, inp, target):
        ...
        record = {
            'loss': loss.item(),
            'top1': accuracy(output, target, topk=(1,))[0] / output.size(0),  # 明确指定topk
            'top5': accuracy(output, target, topk=(5,))[0] / output.size(0),  # 单独获取top5
            'learning_rate': self.lr.get_lr()[0]
        }
